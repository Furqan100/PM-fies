...
<body class="bg-white">
    ...
    <div class="container mx-auto px-4 py-10">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-black text-center flex-grow">Project List</h1>
            @if(auth()->user()->isAdmin())
            <a href="{{ route('projects.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-600 transition"> + Create Project</a>
            @endif
        </div>
        ...
        @else
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($projects as $project)
                    <div class="p-6 rounded-xl shadow-lg bg-gray-800 border-2 border-gray-500 hover:border-yellow-400 transition-all duration-300 transform hover:scale-105">
                        <h2 class="text-2xl font-bold text-gray-200">{{ $project->name }}</h2>
                        <a href="{{ route('projects.show', $project->id) }}" class="mt-4 inline-block text-blue-400 font-semibold hover:text-blue-300 transition duration-300 ease-in-out">
                            View Details
                        </a>
